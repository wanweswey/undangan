...
export const guest = (() => {
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'The Wedding of Wawan and Fatmawati',
            dates: `${formatDate('2026-06-05 10:00')}/${formatDate('2026-06-05 11:00')}`,
            details: 'Tanpa mengurangi rasa hormat, kami mengundang Anda untuk berkenan menghadiri acara pernikahan kami. Terima kasih atas perhatian dan doa restu Anda, yang menjadi kebahagiaan serta kehormatan besar bagi kami.',
            location: 'Jl. Cibogo Blok Balimbing RT 02/04 Dekat Pesantren Rancaherang.',
            ctz: config.get('tz'),
        });
    ...
    const formatDate = (d) => {
    const safeDate = d.replace(' ', 'T');
    const parsed = new Date(safeDate);
    if (isNaN(parsed)) {
        console.error('Invalid date:', d);
        return '';
    }
    return parsed.toISOString().replace(/[-:]/g, '').split('.').shift();
};
